...
Program utama: baca sensor, kalkulasi, TFT, web server.
config.h
Semua pin, konstanta kalibrasi, kredensial WiFi. Ubah di sini.
index_html.h
Dashboard live (PROGMEM) yang disajikan ESP32.
HEART_RATE_REVERSE_ENGINEERING.md
Panduan membaca sinyal HR bawaan Bodymax.
...
ESP32
VCC
3V3 (atau 5V bila encoder butuh 5V + level shifter untuk sinyal)
GND
GND
A (out A)
GPIO25
...
Encoder PPR — isi ENCODER_PPR sesuai datasheet. Verifikasi: kayuh 10 putaran penuh, cek encoder.getCount() ≈ PPR*4*10.
VL53L0X level — buka Serial Monitor, baca distMm:
...
Speed — sesuaikan WHEEL_CIRCUMFERENCE_M dan DRIVE_RATIO_WHEEL_PER_CRANK agar terasa realistis (cadence 60 rpm ≈ 25–30 km/j pada rasio umum).
Torsi — TORQUE_BASE_NM, TORQUE_PER_LEVEL_NM, TORQUE_CADENCE_GAIN. Untuk akurat, bandingkan dengan power meter komersial dan sesuaikan konstanta.
...
Cara upload
Buka StaticBikeMonitor.ino di Arduino IDE.
Instal semua library di atas + atur User_Setup.h TFT_eSPI.
Pilih board "ESP32 Dev Module", pilih port, klik Upload.
Buka Serial Monitor (115200) untuk melihat IP.
